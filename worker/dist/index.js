...
// ../../../../../AppData/Roaming/npm/node_modules/wrangler/node_modules/unenv/dist/runtime/npm/inherits.mjs
...
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
var import_web_push = __toESM(require_src());
var GITHUB_AUTH = "https://github.com/login/oauth/authorize";
var GITHUB_TOKEN = "https://github.com/login/oauth/access_token";
var FALLBACK_REDIRECT = "https://semnanplatform.ir/admin/index.html";
var ORIGIN = "https://semnanplatform.ir";
var K_SUBS = "notif:subs";
var K_IDS = "notif:snapshot_ids";
var MAX_SUBS = 2e3;
function htmlPage(title2, body) {
  return new Response(
...
__name(json, "json");
async function getSubs(env2) {
  const v = await env2.NOTIF_SUBS.get(K_SUBS, "json");
  return Array.isArray(v) ? v : [];
}
__name(getSubs, "getSubs");
async function setSubs(env2, subs) {
  await env2.NOTIF_SUBS.put(K_SUBS, JSON.stringify(subs.slice(0, MAX_SUBS)));
}
__name(setSubs, "setSubs");
async function getIds(env2) {
  const v = await env2.NOTIF_SUBS.get(K_IDS, "json");
  return Array.isArray(v) ? v : null;
}
__name(getIds, "getIds");
async function setIds(env2, ids) {
  await env2.NOTIF_SUBS.put(K_IDS, JSON.stringify(ids));
}
__name(setIds, "setIds");
function toPayload(it, origin) {
  const title2 = it.title || "";
  const url = it.link && /^https?:/.test(it.link) ? it.link : origin + (it.link || "/");
  let ntitle, body;
  if (it.type === "course") {
    ntitle = "\u062F\u0648\u0631\u0647\u200C\u06CC \u0622\u0645\u0648\u0632\u0634\u06CC \u062C\u062F\u06CC\u062F \u{1F4DA} " + title2;
    body = it.teacher ? it.teacher + (it.price ? " \xB7 " + it.price : "") : it.summary || "";
  } else if (it.type === "discount") {
    ntitle = "\u062A\u062E\u0641\u06CC\u0641 \u062C\u062F\u06CC\u062F \u{1F381} " + title2;
    body = it.summary || "";
    if (it.code) body = (body ? body + " \u2014 " : "") + "\u06A9\u062F \u062A\u062E\u0641\u06CC\u0641: " + it.code;
  } else {
    ntitle = "\u0627\u0637\u0644\u0627\u0639\u06CC\u0647\u200C\u06CC \u062C\u062F\u06CC\u062F \u{1F4E3} " + title2;
    body = it.summary || "";
  }
  return {
    id: it.id,
...
async function sendPush(sub, payload, env2) {
  ...
  await import_web_push.default.sendNotification(sub, JSON.stringify(payload), options);
}
__name(sendPush, "sendPush");
async function processNewItems(env2) {
  const origin = env2 && env2.SITE_ORIGIN || ORIGIN;
  let latest = null;
  try {
...
__name(processNewItems, "processNewItems");
async function removeSub(env2, endpoint) {
  const subs = await getSubs(env2);
  const next = subs.filter((s) => s.endpoint !== endpoint);
  if (next.length !== subs.length) await setSubs(env2, next);
}
...
var index_default = {
  async fetch(request3, env2) {
    ...
    if (request3.method === "OPTIONS" && pathname.startsWith("/api/")) {
      return new Response(null, { status: 204, headers: CORS });
    }
    if (pathname === "/api/subscribe" && request3.method === "POST") {
      let body;
      try {
      ...
      await setSubs(env2, subs);
      return json({ ok: true, subs: subs.length });
    }
    if (pathname === "/api/unsubscribe" && request3.method === "POST") {
      let body;
      try {
        body = await request3.json();
      } catch (_) {
    ...
    if (pathname === "/api/_cron") {
      if (!env2.ADMIN_KEY || searchParams.get("key") !== env2.ADMIN_KEY) {
        return json({ error: "forbidden" }, 403);
      }
      await processNewItems(env2);
      return json({ ok: true });
    }
    if (pathname === "/api/_testpush") {
      ...
      const payload = {
        id: "test:manual",
        title: "\u{1F9EA} \u062A\u0633\u062A \u0627\u0639\u0644\u0627\u0646 \u067E\u0633\u200C\u0632\u0645\u06CC\u0646\u0647",
      ...
      for (const sub of targets) {
        try {
          await sendPush(sub, payload, env2);
    ...
    if (pathname === "/auth") {
      const client_id = searchParams.get("client_id") || env2.GITHUB_CLIENT_ID;
      ...
      if (!client_id || !env2.GITHUB_CLIENT_SECRET) {
    ...
    if (pathname === "/auth/callback") {
      ...
      const client_id = env2.GITHUB_CLIENT_ID;
      const client_secret = env2.GITHUB_CLIENT_SECRET;
      ...
      back.hash = `access_token=${encodeURIComponent(data.access_token)}` + (data.scope ? `&scope=${encodeURIComponent(data.scope)}` : "") + `&token_type=bearer` + (decapState ? `&state=${encodeURIComponent(decapState)}` : "");
  ...
  async scheduled(_event, env2, ctx) {
    ctx.waitUntil(processNewItems(env2));
  }
};
export {
